...
for (const item of SIDEBAR_ITEMS.filter((item) => item.key !== "home")) {
	destinations.get(item.href, (c) =>
		c.html(
			<Layout title={`${item.label} · Tally`} active={item.key} demo={true}>
...
destinations.get("/more", (c) =>
	c.html(
		<Layout title="More · Tally" active="more" demo={true}>
