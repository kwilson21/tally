...
| MoneyInput | A dollar amount field (#66): `$` and a text field (`inputmode="decimal"`), then a row of four 44px buttons −$1 · −1¢ · +1¢ · +$1, then chips: "Round up to $X" (only with cents) and "Last month: $X" (dimmed when the field already holds it). The minus buttons are disabled at $0. money.js runs it; without it the buttons and chips are hidden. |
